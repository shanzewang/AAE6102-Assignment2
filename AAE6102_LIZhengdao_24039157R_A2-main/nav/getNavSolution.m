...
% function [obs, sat, navSolution, collect_fault] = getNavSolution(obs, sat, navSolution, allSettings, collect_fault)
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
if (trynav)
    ...
    [Pos, obs] = calcPosLSE(obs, sat, allSettings, Pos); 
    % [Pos, collect_fault] = calcPosLSE(obs, sat, allSettings, Pos, collect_fault);
    ...
    % Update observation structure (if not using skymask)
    if allSettings.sys.skymask_type ~= 1
        obs = updateObservations(obs, Pos, Vel, allSettings);
    end

    % Coordinate conversion 
    if ~Pos.whether_abort_operation
